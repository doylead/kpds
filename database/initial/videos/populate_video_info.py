...
for result in channel_results:
    ...
    while morePages:
        ...
        for ji in jn['items']: # Loop over the videos
            ...
            length = 0 # Not accessible in these API calls
            tracking = False # Do not track by default

            insertstring = ('INSERT INTO "YouTube_Videos" '
                '(song_id,youtube_channel_id,video_type_id,published_datetime,discovered_datetime,video_title,video_description,youtube_video_external_key,length,tracking) '
                'values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)')
            line_list = (song_id,youtube_channel_id,video_type_id,published_datetime,discovered_datetime,video_title,video_description,youtube_video_external_key,length,tracking)
